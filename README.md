# RAG Research Studio â€” Frontend

A premium Next.js frontend for the RAG Research Studio â€” a production-grade Retrieval-Augmented Generation system over 18 curated research papers.
...
### Landing Page
![Landing Page](screenshots/screenshotslanding.png)
*Centered hero with floating glass paper cards reacting to mouse movement. Cards are populated from the live backend API â€” real paper titles, authors, and years.*

### Chat Interface
![Chat Interface](screenshots/screenshotschat.png)
...
### Papers Library
![Papers Library](screenshots/screenshotspapers.png)
...
**Premium AI startup aesthetic** â€” inspired by Linear, Framer, and Perplexity AI.

- **#050505** matte black background â€” not pure black, avoids harshness
- **#FF6A00** burnt orange accent â€” used only for meaningful highlights, never decorative
- **Space Grotesk** â€” premium sans-serif with tight letter-spacing on large headings
...
- No fake metrics or placeholder data â€” everything from the real backend
- No enterprise features that don't exist in the backend (no workspaces, no similarity scores, no complex filters)
- No generic Tailwind template feel â€” all layout uses custom inline styles with precise values
...
- 6 floating glass paper cards on left/right with mouse parallax â€” populated from `/api/papers`
...
| Framer Motion | Animations â€” card reveals, retrieval sequence, page transitions |
| Lenis | Smooth scroll |
...
The frontend is a pure UI layer â€” it calls the FastAPI backend for all data. No mock responses anywhere.

```
Landing page  â†’  GET /api/papers     (paper grid + floating cards)
Chat page     â†’  POST /api/query     (question â†’ answer + citations + key_takeaways + latencies)
Papers page   â†’  GET /api/papers     (full searchable library)
```
